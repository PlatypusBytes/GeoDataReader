...
def test_len_cpts():
    ...
    radius_distance = 0.2
    cpts = read_BRO.read_cpts(location, radius_distance, output_dir="cpts", interpret_cpt=False)
    shutil.rmtree("cpts")
    assert len(cpts) == 4
    ...
    cpts = read_BRO.read_cpts(location, radius_distance, start_date=start_date,output_dir="cpts", interpret_cpt=False)
    shutil.rmtree("cpts")
    assert len(cpts) == 10
